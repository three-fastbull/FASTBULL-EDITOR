...
def test_scaffold_workspace_generates_html_and_assets(tmp_path: Path):
    ...
    # Runtime dependencies are staged locally so checks/renders do not depend
    # on a public CDN and Thai copy never falls back to missing-glyph boxes.
    assert (workspace / "assets" / "gsap.min.js").is_file()
    assert 'src="assets/gsap.min.js"' in html
    assert "cdn.jsdelivr.net/npm/gsap" not in html
    assert (workspace / "assets" / "NotoSansThai-Variable.ttf").is_file()
    assert "OpenMontage Thai Fallback" in html
