...
class HyperFramesCompose(BaseTool):
    ...
    def _scaffold(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        # Prefer the bundled GSAP runtime so validation and rendering remain
        # deterministic when the machine is offline or a public CDN is blocked.
        # The CDN remains a fallback for stripped-down source distributions.
        gsap_script_src = _GSAP_CDN_URL
        bundled_gsap = (
        ...
        if bundled_gsap.is_file():
            shutil.copy2(bundled_gsap, assets_dir / "gsap.min.js")
            gsap_script_src = "assets/gsap.min.js"

        # Bundle a Thai-capable fallback font. Chromium images used for local
        # rendering often ship with Latin-only system fonts, which otherwise
        # turns Thai captions into missing-glyph boxes.
        thai_font_src: Optional[str] = None
        bundled_thai_font = (
            Path(__file__).resolve().parents[2]
            / "assets"
            / "fonts"
        ...
        if bundled_thai_font.is_file():
            thai_font_name = "NotoSansThai-Variable.ttf"
            shutil.copy2(bundled_thai_font, assets_dir / thai_font_name)
            thai_font_src = f"assets/{thai_font_name}"
        ...
        html = self._generate_index_html(
            ...
            gsap_script_src=gsap_script_src,
            thai_font_src=thai_font_src,
        )
    ...
    def _generate_index_html(
        ...
        if thai_font_src:
            thai_font_face = (
                '@font-face { font-family: "OpenMontage Thai Fallback"; '
                f'src: url("{self._escape_attr(thai_font_src)}") format("truetype"); '
...
<head>
  ...
  <style>
    ...
    body {{ margin: 0; background: var(--color-bg); color: var(--color-fg); font-family: var(--font-body), "OpenMontage Thai Fallback", sans-serif; }}
    ...
    .clip.text-card h1 {{ font-family: var(--font-heading), "OpenMontage Thai Fallback", sans-serif; font-weight: 700; font-size: 96px; line-height: 1.1; margin: 0; color: var(--color-fg); }}
    .clip.text-card .subtitle {{ font-size: 36px; margin-top: 24px; color: var(--color-accent); }}
  </style>
  <script src="{self._escape_attr(gsap_script_src)}"></script>
</head>
<body>
